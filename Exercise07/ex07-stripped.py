...
# note I changed the function spec because it did not refer to correct vars
def sentence_score_f_a_given_e(sentenceE, sentenceF, alignment, tokenProbabilityFGivenE):
    '''
        ...
        tokenProbabilityFGivenE: a word-keyed dict of word-keyed dicts of probabilities
            The translation probabilities of a foreign token given the source (English) token,
            that only contains entries for the non-zero (NZ) translations.
    ...
    for i in range(m):
        ...
        f_word = sentenceF[i]
        e_word = sentenceEN[alignment[i]]
        t_fe = tokenProbabilityFGivenE.get(e_word).get(f_word)
        # probability is product of word probs
        prob *= t_fe
    return prob
    
def sentence_score_f_given_e(sentenceE, sentenceF, tokenProbabilityFGivenE):
    '''
    ...
    '''
    alignmentsValid = get_valid_alignments(sentenceE, sentenceF, tokenProbabilityFGivenE)
    prob = 0
    '''######### TODO: ######### (DONE)
     Complete this function.
    ...
    for align in alignmentsValid:
        ...
        prob += sentence_score_f_a_given_e(sentenceE, sentenceF, align, tokenProbabilityFGivenE)
    return prob
...
# note that the dict passed in is tFGENZ but it was originally called tEGFNZ
# this is extremely sloppy, so I changed the variable names
def rank_valid_alignments(sentenceE, sentenceF, tFGENZ):
    '''
        ...
        iterable of
            ...
            e.g.: ( ((0,1),0.5), (-1,0), 0.4 ), ... )
              ...
              score ------- +-------------+      
    '''
    alignments = get_valid_alignments(sentenceE, sentenceF, tFGENZ)

    '''######## TODO ######### (DONE)
    Complete this function.
    You can should be able to do this in no more that 2-3 lines.
    '''
    # add null token to English sentence
    sentenceEN = list(sentenceE) + ['NULL']
    # keep track of (alignment, score)
    scoreAlignments = []
    # look at each valid alignment
    for align in alignments:
        # give each a score by multiplying transition probs for each word pair
        t_ef = 1
        for i in range(len(align)):
            t_ef *= tFGENZ.get(sentenceEN[align[i]]).get(sentenceF[i])
        # append to list
        scoreAlignments.append((align, t_ef))
...
def rank_valid_translations(sentenceE, m, tFGENZ):
    '''
    ...
    '''
    scoreSentences = []
    # look at each of the valid translations
    for sentenceF in sentencesF:
        # get the score
        curr_score = sentence_score_f_given_e_fast(sentenceE, sentenceF, tFGENZ)
        scoreSentences.append((sentenceF, curr_score))
    # sort the scores
...
if __name__ == "__main__":
    ...
    sentenceF1_star = ('die', 'pferd', 'rennt', 'schnell')
    sentenceF1_prime = ('das', 'pferd', 'rennt', 'schnell')
    ...
    '''
    # unigram prob dict -- start from counts
    modelUnigrams = unigramsForeign
    # divide each word by total number of words
    for word in modelUnigrams:
        modelUnigrams[word] /= wordsForeign
    # bigram prob dict -- start from counts
    modelBigrams = bigramsForeign
    # get the counts of the first word in the bigram
    for word1 in modelBigrams:
        count_word1 = sum(modelBigrams[word1].values())
        # normalize by count(word1)
        for word2 in modelBigrams[word1]:
            modelBigrams[word1][word2] /= count_word1
    
    print('Ranking language model probabilities:')
    rankingSentences1 = tuple( (sentence,sentence_probability_wrt_bigram_model(sentence, modelUnigrams, modelBigrams))
                              for sentence in (sentenceF1, sentenceF1_star, sentenceF1_prime) )
    rankingSentences2 = tuple( (sentence,sentence_probability_wrt_bigram_model(sentence, modelUnigrams, modelBigrams))
                              for sentence in (sentenceF2b, sentenceF2a, sentenceF2c) )
    print_ranking(rankingSentences1, k=None)
    print_ranking(rankingSentences2, k=None)
